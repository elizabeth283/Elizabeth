...
function App() {
  ...
  const [refugioCreado, setRefugioCreado] = useState(false);
  ...
  async function ClientCall(params) {
    ...
    try {
      ...
      if (esLectura) {
        ...
        if (params.funcion === "retornar_todo" && decoded && decoded.length >= 5) {
          cambiarRespuesta(`🐾 Adoptante: ${decoded[4]}\n📅 Año de registro: ${decoded[0]}\n⭐ Nivel del adoptante: ${decoded[3]?.['raw']?.[1] || 'N/A'}% de beneficios\n🏠 Dirección: ${decoded[1]}`);
        }
        return decoded;
      }
          ...
          onSuccess: async (txres) => {
            ...
            if (params.funcion === "crear_empresa") {
              ...
              if (id) {
                ...
                setRefugioCreado(true);
              }
            }
          },
          onError: (err) => {
            alert("Error al procesar la adopción: " + err.message);
          },
        }
      );
    } catch (error) {
      ...
      <header className="adoption-header">
        <div className="brand">
          ...
          <div className="brand-text">
            <h1>AdoptFriend</h1>
            <div className="subtitle">Sistema de Adopción de Animales</div>
          </div>
        </div>
        <ConnectButton />
      </header>
      ...
      <main className="main-content">
        {!cuenta ? (
          <div className="hero-section">
            <h1 className="hero-title">AdoptFriend</h1>
            <p className="hero-subtitle">
              Conecta animales necesitados con hogares amorosos. 
              Gestiona adopciones de forma segura y transparente con blockchain.
            </p>
