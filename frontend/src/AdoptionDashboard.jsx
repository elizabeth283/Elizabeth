...
export function AdoptionDashboard({ ClientCall, estado, objectId, setObjectId, respuesta }) {

    return (
        <div>
            {/* Header del Dashboard */}
            <div className="hero-section">
                <h1 className="hero-title">Centro de Adopciones</h1>
                <p className="hero-subtitle">
                    Gestiona adoptantes, animales y consulta el historial completo de adopciones.
                </p>
                
                <div style={{maxWidth: '600px', margin: '0 auto'}}>
                    <div className="form-group">
                        <label className="form-label">ID del Refugio</label>
                        <input 
                            type="text" 
                            placeholder="Pega aquí el ID del Refugio (0x...)"
            ...
            {respuesta !== null && (
                <div className="result-panel">
                    <h3 className="result-title">📊 Información del Adoptante</h3>
...
function AdoptionFunctionCard({ config, ClientCall, estado, objectId }) {
    const [valores, setValores] = useState({});

    function enviar(e) {
        e.preventDefault();
        if (!objectId) {
            alert("Primero debes ingresar el ID del refugio");
            return;
        }
    ...
    // Iconos para funciones de adopción
